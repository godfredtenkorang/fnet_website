...
def add_mileage(request):
    ...
    # driver = Driver.objects.filter(driver=request.user).first()
    driver = get_object_or_404(Driver, driver=request.user)
    if request.method == 'POST':
        form = MileageRecordForm(request.POST, instance=driver)
        if form.is_valid():
...
def view_mileage(request):
    driver = request.user
    records = MileageRecord.objects.filter(driver=driver.id).order_by('-date')
